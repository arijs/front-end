...
exports["default"] = diffDates;

function singularPlural(n, s) {
  return s instanceof Array ? s[n == 1 ? 0 : 1] : s;
}

function diffDates(d1, d2, labels, secondsHide) {
  var p = true;
  var t1 = d1.getTime();
  var t2 = d2.getTime();
  ...
  oac.maxPart = oac.y ? 'y' : oac.M ? 'M' : oac.d ? 'd' : oac.h ? 'h' : oac.m ? 'm' : oac.s ? 's' : oac.ms ? 'ms' : null;
  var s = [];
  ...
  if (oac.d != 0
  /*|| s.length == 0*/
  ) s.push(oac.d + singularPlural(Math.abs(oac.d), labels.dia));
  oac.sd = s.join(' ');
  var st_m = oac.m;
  var st_s = oac.s;
