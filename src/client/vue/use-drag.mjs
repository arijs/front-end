...
import { getPointerDragInstance } from '../dom/pointer-drag.mjs';

export default function useDrag(opt = {}) {
	// function fnLimit(min, max, val) {
	// 	return Math.max(min, Math.min(max, val));
	// }
	var {
		name,
		apiDrag = getPointerDragInstance(),
		ease = cubic,
		easeMod = modOut,
		easeAuto = ease,
		easeAutoMod = easeMod,
	...
	var fnMod = function(i, mod) {
		if (!mod) {
			console.warn(`PointerDrag${name}: mod should not be empty nor zero`);
		}
	...
	var fnAnimateToPos = function(targetPos, resetTime) {
		...
		animate(pos, targetPos, time, ease, easeMod, function(val, pos) {
			if (!stop) fnPos(val);
	...
	var fnEnd = function() {
		...
		if (obj.debug) {
			console.log(`pointerDrag${name}::fnEnd move:`, move, rmove, amove, tmove, tmove*wf, mmod, '/', wf, wfp, '/', pos, posMove, posTarget, posRound);
		}
	...
	var fnAutoSlide = function(timeWait, timeAnim, timeManual) {
		...
		function next() {
			...
			animate(pos, target, timeAnim, easeAuto, easeAutoMod, function(val, pos) {
