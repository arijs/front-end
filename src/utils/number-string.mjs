...
export function numberParse(n) {
	return parseFloat(n.replace(/\./g, '').replace(/,/g, '.'));
}
export function numberFormat(n, dlen, dsep, gsep, glen) {
	if (isNaN(dlen)) {
...
export function numberFormatStr(n, dlen, dsep, gsep, glen) {
	var sign = n.match(/[+-]/);
	sign = sign && sign[0] || '';
	n = n.replace(/[^0-9]/g, '');
	n = n.replace(/^0+/, '');
...
export function numberFormatArr(n, dlen, dsep, gsep, glen) {
	if (!glen) glen = 3;
	if (null == dsep) dsep = ',';
	if (null == gsep) gsep = '.';
	var groups = [];
